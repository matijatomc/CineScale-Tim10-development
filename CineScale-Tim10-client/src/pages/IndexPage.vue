  ...
  <div id="indexpage" style="display: flex; flex-direction: column; align-items: center;">
    ...
    <div class="content" style="display: flex; flex-direction: column; align-items: center; width: 80%;">
      <div v-for="(item, index) in searchResults" :key="index"
        style=" background-color: lightgray; border: 1px solid black; border-radius: 8px; padding: 20px; margin: 10px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2); width: 100%;">
        ...
        <div v-if="addReviewMovieId === item.id"
        ...
        <button @click="getMovieDetails(item.id)" style="padding: 5px 10px; margin-top: 10px;">Show reviews</button>
        <div v-if="selectedMovie && selectedMovieId === item.id"
          style=" background-color: #b5b1b1; border: 1px solid black; border-radius: 8px; padding: 20px; margin-top: 10px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2);">
...
export default {
  ...
  methods: {
    ...
    async addReview (id, name) {
      ...
      try {
        await axios.post('http://localhost:3000/recenzije?collection=film&movieOrSeriesId=' + id + '&komentar=' + this.newReview.komentar + '&ocjena=' + this.newReview.ocjena + '&korisnikUID=' + this.userId)
        this.addReviewMovieId = null
